...
export function StudyReader({
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-[#17141F] text-white' : 'bg-[#FAF7F1] text-[#1E1B2E]'}`}>
      <div className={`sticky top-0 z-10 flex items-center justify-between px-4 py-3 border-b ${
        ...
        <Link href="/studies" className={darkMode ? 'text-white' : 'text-[#2E2860]'}>← Voltar</Link>
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setDarkMode((d) => !d)} className={`text-lg ${darkMode ? 'text-[#E0A943]' : 'text-[#2E2860]'}`}>
            {darkMode ? '☀' : '🌙'}
          </button>
          <button onClick={handleShare} className={darkMode ? 'text-white' : 'text-[#2E2860]'}>↗</button>
        </div>
      </div>

      <article className="px-5 py-6 max-w-2xl mx-auto">
        {category && <Badge variant="indigo" className="mb-3">{category.name}</Badge>}
        ...
        {preacher && (
          <Link href={`/profile/${preacher.slug}`} className="flex items-center gap-3 mb-4">
            ...
            <div>
              <p className={`font-medium text-sm ${darkMode ? 'text-white' : 'text-[#1E1B2E]'}`}>{preacher.display_name}</p>
              {preacher.church && (
                <p className={`text-xs ${darkMode ? 'text-white/50' : 'text-[#8A8797]'}`}>{preacher.church}</p>
              )}
            </div>
          </Link>
        )}
        ...
        {study.youtube_url && (
          <div className="mb-6 aspect-video rounded-xl overflow-hidden">
            <iframe src={study.youtube_url.replace('watch?v=', 'embed/')} className="w-full h-full" allowFullScreen />
          </div>
        )}

        {study.body && (
          <div
            className={`prose prose-p:leading-relaxed max-w-none ${fontSizeClass} ${darkMode ? 'prose-invert' : ''}`}
