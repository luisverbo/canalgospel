...
export function StudyReader({
  ...
  return (
    <div className="min-h-screen bg-[#FAF7F1] dark:bg-[#17141F] text-[#1E1B2E] dark:text-[#D8D5E4]">
      ...
      {/* Audio — podcast-style layout */}
      {isAudio ? (
        <div className="px-5 py-8 max-w-lg mx-auto flex flex-col items-center gap-6">
          {/* Cover art */}
          <div className="w-56 h-56 rounded-3xl overflow-hidden shadow-lg bg-[#2E2860]">
            {study.cover_url ? (
              <img src={study.cover_url} alt={decodeHtml(study.title)} className="w-full h-full object-cover" />
            ) : (
          ...
          <div className="text-center w-full">
            {category && (
              <span className="inline-block text-[11px] font-semibold px-2.5 py-1 rounded-full mb-2 bg-[#2E2860]/8 dark:bg-[#2E2860]/60 text-[#2E2860] dark:text-[#B5B0D8]">
                {decodeHtml(category.name)}
              </span>
            )}
            <h1 className="text-xl font-semibold leading-snug text-[#1E1B2E] dark:text-[#F3F1FA]">
              {decodeHtml(study.title)}
            </h1>
            {preacher && (
              <Link href={`/profile?slug=${preacher.slug}`}
                ...
                {preacher.photo_url ? (
                  <img src={preacher.photo_url} alt={decodeHtml(preacher.display_name)}
                    className="h-5 w-5 rounded-full object-cover border border-[#2E2860]/20 dark:border-[#E0A943]/40 shrink-0" />
                ) : (
                  <span className="h-5 w-5 rounded-full bg-[#2E2860]/10 dark:bg-[#E0A943]/20 flex items-center justify-center text-[9px] font-bold text-[#2E2860] dark:text-[#E0A943] shrink-0">
                    {preacher.display_name[0]}
                  </span>
                )}
                <span className="text-sm font-semibold text-[#2E2860] dark:text-[#E0A943]">{decodeHtml(preacher.display_name)}</span>
              </Link>
            )}
          </div>
          ...
          {study.audio_url && (
            <div className="w-full">
              <audio controls src={study.audio_url} className="w-full rounded-xl" />
            </div>
          )}

          {/* Description if present */}
          {study.body && study.body.trim() && study.body.trim() !== ' ' && (
            <div className="w-full text-sm text-[#8A8797] dark:text-white/50 text-center leading-relaxed">
              {looksLikeHtml(study.body)
                ? <span dangerouslySetInnerHTML={{ __html: decodeHtml(study.body) }} />
                : study.body
              }
            </div>
          )}
        ...
        <article className="px-5 py-6 max-w-2xl mx-auto">
          ...
          {/* Supplemental audio player for video/text that also has audio */}
          {study.audio_url && (
            <div className="my-6 bg-[#2E2860]/6 dark:bg-[#2E2860]/30 rounded-2xl p-4">
              ...
              <audio controls src={study.audio_url} className="w-full" />
            </div>
          )}
          ...
          {study.body && study.body.trim() && study.body.trim() !== ' ' && (
            looksLikeHtml(study.body) ? (
              ...
              <div className={`${fontSizeClass} leading-relaxed text-[#1E1B2E] dark:text-[#D8D5E4]`}>
                {linkifyPlainText(study.body, 'text-[#2E2860] dark:text-[#E0A943] underline break-words')}
              </div>
            )
          )}
        </article>
