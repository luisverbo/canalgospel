...
export function DevotionalForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    fd.set('verse_ref', form.verse_ref)
    fd.set('verse_text', form.verse_text)
    fd.set('reflection', form.reflection)
    fd.set('youtube_url', form.youtube_url)

    const result = await createDevotional(fd)
    if (result?.error) {
    ...
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-[#1E1B2E]/8 p-5 flex flex-col gap-4">
      ...
      <Field label="Data">
        <input type="date" required value={form.date}
          onChange={(e) => set('date', e.target.value)} className="field-input" />
      </Field>

      <Field label="Referência do Versículo">
        <input required value={form.verse_ref}
          onChange={(e) => set('verse_ref', e.target.value)} className="field-input"
          placeholder="Ex: João 3:16" />
      </Field>

      <Field label="Texto do Versículo">
        <textarea required value={form.verse_text}
          onChange={(e) => set('verse_text', e.target.value)} className="field-input"
          rows={3} placeholder="Texto completo do versículo..." />
      </Field>

      <Field label="Reflexão (opcional)">
        <textarea value={form.reflection}
          onChange={(e) => set('reflection', e.target.value)} className="field-input"
          rows={4} placeholder="Mensagem de reflexão..." />
      </Field>

      <Field label="YouTube (opcional)">
        <input type="url" value={form.youtube_url}
          onChange={(e) => set('youtube_url', e.target.value)} className="field-input"
      ...
      <style jsx>{`
        ...
        .field-input:focus { border-color: #2E2860; }
      `}</style>
    </form>
  )
}
